...
const LOWEST_SUPPORTED_RUNNER_VERSION = '0.18.0';
const LOWEST_UNSUPPORTED_RUNNER_VERSION = '0.19.0';
...
/* c8 ignore start */
export class TTSLPythonServer {
    private readonly logger: TTSLLogger;
    private readonly messaging: TTSLMessagingProvider;
    private readonly settingsProvider: TTSLSettingsProvider;
    ...
    private async getValidRunnerCommand(): Promise<string | undefined> {
        ...
        try {
            installedVersion = await this.getInstalledRunnerVersion(await command);
            this.logger.debug(`Found ttsl-runner with version "${installedVersion}".`);
        } catch (error) {
            await this.reportBadRunnerCommand(await command, error);
            return undefined;
        }
    ...
    private async getLatestMatchingRunnerVersion(): Promise<string | undefined> {
        // Get information about `ttsl-runner` from Pypi
        const response = await fetch('https://pypi.org/pypi/ttsl-runner/json', {
            signal: AbortSignal.timeout(2000),
        });
        if (!response.ok) {
            this.logger.error(`Could not fetch the latest version of ttsl-runner: ${response.statusText}`);
            return undefined;
        }
