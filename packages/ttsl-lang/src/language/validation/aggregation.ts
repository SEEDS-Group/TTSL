        ...
        ids?.forEach((id) => {
            if (id !== undefined) {
                ...
                if (!isGrouped) {
                    accept(
                        'error',
                        `The Function '${node.name}' needs a grouped Parameter or an aggregation in the function body to aggregate over '${id}'.`,
                        {
                            node,
...
const hasAggregation = (statements: TslStatement[] | undefined, id: string, prev: boolean = false) => {
    ...
    statements.forEach((stmt) => {
        if (
            isTslExpressionStatement(stmt) &&
            isTslAggregation(stmt.expression) &&
            stmt.expression.groupedBy.id.map((elmId) => elmId.target.ref?.name).includes(id)
        ) {
            result = true;
        } else if (
            isTslAssignment(stmt) &&
            isTslAggregation(stmt.expression) &&
            stmt.expression.groupedBy.id.map((elmId) => elmId.target.ref?.name).includes(id)
        ) {
            result = true;
    ...
    statements.forEach((elm) => {
        if (isTslConditionalStatement(elm)) {
            result =
                hasAggregation(elm.ifBlock.statements, id, result) &&
                hasAggregation(elm.elseBlock?.statements, id, result);
