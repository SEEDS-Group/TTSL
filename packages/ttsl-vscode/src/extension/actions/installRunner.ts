...
import { dependencies, rpc } from '@ttsl/lang';
import { LanguageClient } from 'vscode-languageclient/node.js';
import { ttslLogger } from '../helpers/logging.js';
...
const doInstallRunner = async (runnerVirtualEnvironmentUri: Uri): Promise<boolean> => {
    ...
    if (!pythonCommand) {
        ...
        ttslLogger.error('Could not find a matching Python interpreter.');
        return false;
    }
...
export const installRunnerInVirtualEnvironment = async (pipCommand: string): Promise<void> => {
    return new Promise((resolve, reject) => {
        const installCommand = `"${pipCommand}" install --upgrade "ttsl-runner${dependencies['ttsl-runner'].pipVersionRange}"`;
        const process = child_process.spawn(installCommand, { shell: true });

        process.stdout.on('data', (data: Buffer) => {
            ttslLogger.debug(data.toString().trim());
        });
        process.stderr.on('data', (data: Buffer) => {
            ttslLogger.error(data.toString().trim());
        });
...
const getRunnerCommand = (runnerVirtualEnvironmentUri: Uri): string => {
    if (process.platform === 'win32') {
        return `${runnerVirtualEnvironmentUri.fsPath}\\Scripts\\ttsl-runner.exe`;
    } else {
        return `${runnerVirtualEnvironmentUri.fsPath}/bin/ttsl-runner`;
    }
};
