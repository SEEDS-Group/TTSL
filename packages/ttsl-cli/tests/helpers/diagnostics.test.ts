    ...
    const tests: DiagnosticToStringTest[] = [
        {
            ...
            expected: chalk.red(`test.ttsl:1:1: [error] message`),
        },
        {
            testName: 'warning',
            diagnostic: {
            ...
            expected: chalk.yellow(`test.ttsl:1:1: [warning] message`),
        },
        {
            ...
            expected: chalk.red(`test.ttsl:1:1: [warning] message`),
        },
        {
            testName: 'info',
            diagnostic: {
            ...
            expected: chalk.blue(`test.ttsl:1:1: [info] message`),
        },
        {
            testName: 'hint',
            diagnostic: {
            ...
            expected: chalk.gray(`test.ttsl:1:1: [hint] message`),
        },
        {
            testName: 'with code',
            diagnostic: {
            ...
            expected: chalk.red(`test.ttsl:1:1: [error] message (CODE)`),
        },
    ];

    it.each(tests)('$testName', ({ diagnostic, options, expected }) => {
        const uri = URI.file(path.join(process.cwd(), 'test.ttsl'));
