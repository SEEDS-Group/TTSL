...
export const simulate = async (date: string, data: string, targets: string, fsPaths: string): Promise<void> => {
    ...
    const pythonProcess = spawn('python', [
        'C:\\Users\\tatam\\Documents\\Uni\\Bachelor\\TTSL\\packages\\ttsl-cli\\tests\\resources\\simulate\\generate\\tests\\simulate\\main\\gen_main_simulate.py',
    ]);

    pythonProcess.stdout.on('data', (d) => {
        console.log(String(d)); //print simulation results
    });
